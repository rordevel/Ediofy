      ...
      progress: (e, data) ->
        if data.context
          ...
          if $('.circularProgress').length
            data.context.find('.circularProgress').attr('class', 'circularProgress --' + progress)
            data.context.find('.circularProgress__overlay .progressPerText .perText').text(progress+'%')
        ...
        if (data.files[0].type.indexOf("video") >= 0)
          $('.displayCell').addClass('videoUploadData')
        else
