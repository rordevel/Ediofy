class AddDeletedFalseToConversationsSearchResults < ActiveRecord::Migration[5.0]
  def up
    execute "DROP FUNCTION IF EXISTS conversations_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR, p_sort VARCHAR);
    CREATE OR REPLACE FUNCTION conversations_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest')
      RETURNS TABLE(
        id INTEGER,
        type VARCHAR,
      ...
      BEGIN
        IF p_search IS NULL THEN
          SQL = 'SELECT id, ''conversations'' AS type, 0 AS rank, comments_count, votes_count, view_count
