class AddDistinctToUsersSearchResults < ActiveRecord::Migration[5.0]
  def up
    ...
    CREATE OR REPLACE FUNCTION users_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest')
      ...
      BEGIN
        IF p_search IS NULL THEN
          SQL = 'SELECT DISTINCT id, ''contributors'' AS type, 0 AS rank, comments_count, votes_count, view_count, created_at
            FROM users
            WHERE ediofy_terms_accepted = true AND profile_completed = true AND ghost_mode = false';
        ELSE
          SQL = 'SELECT DISTINCT id, ''contributors'' AS type, TS_RANK_CD(textsearch, query) AS rank, comments_count, votes_count, view_count, created_at
