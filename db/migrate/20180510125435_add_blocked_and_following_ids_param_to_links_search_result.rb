class AddBlockedAndFollowingIdsParamToLinksSearchResult < ActiveRecord::Migration[5.0]
  def up
    execute "DROP FUNCTION IF EXISTS links_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR, p_sort VARCHAR,p_blocked_ids VARCHAR,p_following_ids VARCHAR,p_tags VARCHAR);
    CREATE OR REPLACE FUNCTION links_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest',p_blocked_ids VARCHAR DEFAULT NULL, p_following_ids VARCHAR DEFAULT NULL, p_tags VARCHAR DEFAULT NULL)
      RETURNS TABLE(
        id INTEGER,
        type VARCHAR,
      ...
      BEGIN
        ...
        IF p_search IS NULL THEN
          -- SQL = 'SELECT id, ''links'' AS type, 0 AS rank, comments_count, votes_count, view_count
          --  FROM links WHERE deleted = FALSE AND NOT (user_id = ANY(''' || blocked_ids::text || '''))';
          SQL = 'SELECT id, ''links'' AS type, TS_RANK_CD(textsearch, query) AS rank, comments_count, votes_count, view_count, created_at
