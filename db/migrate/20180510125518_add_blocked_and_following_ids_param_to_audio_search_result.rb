class AddBlockedAndFollowingIdsParamToAudioSearchResult < ActiveRecord::Migration[5.0]
  def up
    execute "DROP FUNCTION IF EXISTS audio_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR, p_sort VARCHAR,p_blocked_ids VARCHAR,p_following_ids VARCHAR,p_tags VARCHAR);
      CREATE OR REPLACE FUNCTION audio_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest',p_blocked_ids VARCHAR DEFAULT NULL, p_following_ids VARCHAR DEFAULT NULL, p_tags VARCHAR DEFAULT NULL)
      RETURNS TABLE(
        id INTEGER,
        type VARCHAR,
      ...
      BEGIN
        ...
        IF p_search IS NULL THEN
          ...
          --  FROM media JOIN media_files ON media_files.media_id = id AND media_files.media_type=''audio''
          ...
          SQL = 'SELECT id, ''media'' AS type, TS_RANK_CD(textsearch, query) AS rank, comments_count, votes_count, view_count, created_at
            ...
            WHERE media_type=''audio'' AND (query @@ textsearch OR (user_id = ANY(''' || following_ids::text || ''')) ) AND NOT (user_id = ANY(''' || blocked_ids::text || '''))';
        ELSE
          SQL = 'SELECT id, ''media'' AS type, TS_RANK_CD(textsearch, query) AS rank, comments_count, votes_count, view_count, created_at
            FROM media_taggings_view, TO_TSQUERY(''' || p_search || '''::text) query
            WHERE media_type=''audio'' AND (''' || p_search || '''::text @@ tags OR query @@ textsearch ) AND NOT (user_id = ANY(''' || blocked_ids::text || '''))';
        END IF;
