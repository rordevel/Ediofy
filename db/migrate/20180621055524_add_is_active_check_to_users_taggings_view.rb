class AddIsActiveCheckToUsersTaggingsView < ActiveRecord::Migration[5.0]
  def up
    self.connection.execute %Q(CREATE OR REPLACE VIEW users_taggings_view AS
      SELECT
        DISTINCT u.id, tg.context,
      ...
      WHERE u.ediofy_terms_accepted = true AND u.profile_completed = true AND u.ghost_mode = false AND u.is_active = true
