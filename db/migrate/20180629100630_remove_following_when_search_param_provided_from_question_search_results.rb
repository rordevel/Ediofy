class RemoveFollowingWhenSearchParamProvidedFromQuestionSearchResults < ActiveRecord::Migration[5.0]

  def up
    execute "DROP FUNCTION IF EXISTS questions_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR, p_sort VARCHAR,p_blocked_ids VARCHAR,p_following_ids VARCHAR,p_tags VARCHAR);
      CREATE OR REPLACE FUNCTION questions_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest',p_blocked_ids VARCHAR DEFAULT NULL, p_following_ids VARCHAR DEFAULT NULL, p_tags VARCHAR DEFAULT NULL)
        RETURNS TABLE(
          id INTEGER,
          type VARCHAR,
        ...
        BEGIN
          ...
          IF p_search IS NULL THEN
            ...
            --  FROM questions
            --   WHERE approved = TRUE AND status <> 2 AND deleted = FALSE AND NOT (user_id = ANY(''' || blocked_ids::text || '''))';
