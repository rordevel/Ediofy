class RemoveFollowingWhenSearchParamProvidedFromPdfSearchResults < ActiveRecord::Migration[5.0]
  def up
    execute "DROP FUNCTION IF EXISTS pdf_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR, p_sort VARCHAR,p_blocked_ids VARCHAR,p_following_ids VARCHAR,p_tags VARCHAR);
      CREATE OR REPLACE FUNCTION pdf_search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest',p_blocked_ids VARCHAR DEFAULT NULL, p_following_ids VARCHAR DEFAULT NULL, p_tags VARCHAR DEFAULT NULL)
      RETURNS TABLE(
        id INTEGER,
        type VARCHAR,
      ...
      BEGIN
        ...
        IF p_search IS NULL THEN
          ...
          --  FROM media JOIN media_files ON media_files.media_id = id AND media_files.media_type=''pdf''
