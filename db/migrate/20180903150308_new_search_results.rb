class NewSearchResults < ActiveRecord::Migration[5.0]
  def up
    ...
    execute "CREATE OR REPLACE FUNCTION search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest')
      ...
      BEGIN
        IF p_search IS NULL THEN
          ...
          SQL_GROUPS = 'SELECT id, ''groups'' AS type, 0 AS rank, created_at
            FROM groups';
        ...
        ELSE
          ...
          SQL_GROUPS = 'SELECT id, ''groups'' AS type, TS_RANK_CD(textsearch, query) AS rank
            FROM groups_taggings_view, TO_TSQUERY(''' || p_search || ''') query
            WHERE query @@ textsearch';

        END IF;

        -- SQL = '(' || SQL_CONTRIBUTORS || ') UNION (' || SQL_CONVERSATIONS || ') UNION (' || SQL_MEDIA || ') UNION (' || SQL_QUESTOINS || ') UNION (' || SQL_LINKS || ') UNION (' || SQL_GROUPS || ') ';
        SQL = '(' || SQL_CONVERSATIONS || ') UNION (' || SQL_MEDIA || ') UNION (' || SQL_QUESTOINS || ') UNION (' || SQL_LINKS || ') UNION (' || SQL_GROUPS || ')';
    ...
    CREATE OR REPLACE FUNCTION search_results(p_limit INTEGER, p_offset INTEGER, p_search VARCHAR DEFAULT NULL, p_sort VARCHAR = 'latest',p_blocked_ids VARCHAR DEFAULT NULL, p_following_ids VARCHAR DEFAULT NULL, p_tags VARCHAR DEFAULT NULL)
      ...
      BEGIN
        ...
        IF p_search IS NULL THEN
          ...
          SQL_GROUPS = 'SELECT id, ''groups'' AS type, TS_RANK_CD(textsearch, query) AS rank, comments_count, votes_count, view_count, created_at
            FROM groups_taggings_view, TO_TSQUERY(''' || p_tags || '''::text) query
            WHERE (query @@ textsearch)';
        ...
        SQL = '(' || SQL_CONVERSATIONS || ') UNION (' || SQL_MEDIA || ') UNION (' || SQL_QUESTIONS || ') UNION (' || SQL_LINKS || ') UNION (' || SQL_GROUPS || ')';
