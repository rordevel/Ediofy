...
class OembedProvider
  class << self
    ...
    def find_provided_from(url)
      ...
      submitted_url_params = Rails.application.routes.recognize_path(url.to_s, method: :get)
      ...
      model_find = OembedProvider.controller_maps[controller]

      if model_find
        model_find_id = model_find[:id]
        model = model_find[:model]
        id = submitted_url_params[model_find_id.to_sym]
      end
